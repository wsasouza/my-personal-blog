...
const FeaturedPostCard = ({ post }) => (
  <div className="relative h-72">
    ...
    <div className="absolute flex h-full w-full flex-col items-center justify-center rounded-lg p-4">
      <p className="text-shadow mb-4 text-xs font-semibold text-lightGray">
        {moment(post.createdAt).format('DD MMM, YYYY')}
      </p>
      <p className="text-shadow mb-4 text-center text-2xl font-semibold text-primary">
        {post.title}
      </p>
      <div className="absolute bottom-5 flex w-full items-center justify-center lg:ml-4">
        <Image
          unoptimized
        ...
        />
        <p className="text-shadow ml-2 inline align-middle font-medium text-secondary">
          {post.author.name}
        </p>
      </div>
    </div>
